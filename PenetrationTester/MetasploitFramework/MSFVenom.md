...
Offer penetration testers a method to quickly craft payloads for different target host architectures and releases while having the possibility to 'clean up' their shellcode so that it does not run into any errors when deployed.
...
```
masterofblafu@htb[/htb]$ msfconsole -q 
...
```
Now we can trigger the `.aspx` payload on the web service by visiting its location.
